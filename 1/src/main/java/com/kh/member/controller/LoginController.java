...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * 2) session : 여기에 담긴 데이터는 직접 지우기전까지, 세션이 만료(서버가 멈추거나, 브라우저 종료)되기 전까지
		 * 				어떤 jsp든, 어떤servlest이건 꺼내서 사용할 수 있음.
		...
		if (loginUser == null) {
			...
			// 해당 방식으로 화면 응답시 url은 http://localhost:8002/jsp/login.me가 나타나며 실제 화면은 localhost:8002/jsp/의 응답화면이 나타나게 된다.
			//응답페이지(jsp)에게 위임시 필요한 객체(RequestDispatcher)
